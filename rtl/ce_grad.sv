...
module ce_grad #(
    ...
    typedef enum logic [3:0] {
        ...
        CE_DIV_T,           // divide all TxV dLogits by T (T*V cycles)
        CE_GRADNORM,        // accumulate ||dLogits||^2 (T*V cycles)
        CE_CLIP,            // compute clip_scale from fp32_sqrt (1 cycle)
        CE_CLIP_APPLY,      // apply clip to all TxV dLogits (T*V cycles)
        CE_DX_INIT,         // dX_init = dLogits * emb (V cycles, parallel over TxD)
        CE_DWOUT,           // dWout   = dLogitsT * x  (T cycles, parallel over VxD)
        DONE_ST
    } state_t;
    ...
    // -----------------------------------------------------------------------
    // FSM (also collects exp_lut results whenever valid_out fires)
    // -----------------------------------------------------------------------
    always_ff @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            case (state)
                ...
                // ----------------------------------------------------------
                // Step 2b - drain last EXP_LAT+1 results (EXP_LAT+1 cycles)
                //   +1 because exp_valid_in_reg is registered (1-cycle extra latency)
                // ----------------------------------------------------------
                CE_ROW_EXP_DRAIN: begin
                    if (cnt == 7'(EXP_LAT)) begin
                        cnt   <= '0;
                        state <= CE_ROW_NORM;
                    end else cnt <= cnt + 1;
                end
                ...
                // ----------------------------------------------------------
                // Step 4 - divide all TxV dLogits by T  (cnt[5:4]=t, cnt[3:0]=v)
                // ----------------------------------------------------------
                CE_DIV_T: begin
